 ...
 */
export function createWfsLoader(
  options: {
    source: VectorSource<Feature<Geometry>>;
    mapZoomGetter: () => number | undefined;
  ...
  return function(extent, resolution, projection, success, failure) {
    const zoom = options.mapZoomGetter() || DEFAULT_ZOOM;
    
    // Skip loading at very low zoom levels
    if (zoom < ZOOM_THRESHOLDS.NO_LOAD) {
      success?.([]);
      return;
    }
    
    // Skip very large areas at high detail zoom
    if (isExtentTooLarge(extent, zoom)) {
      success?.([]);
      return;
    }
    ...
    const featureCount = getFeatureCountForZoom(zoom);
